...
/**
 * Used for MRU files on the File menu. Each MRU menu item is an instance of
 * this action.
 * 
 * @author Glen Smith
 */
public class MRUFileAction extends Action {
	...
	public void run() {
			...
			if (result == SWT.YES) {
				try {
					...
					app.displayErrorDialog(
							Messages.getString("MRUFileAction.ErrorDialog.Title"), e1.getMessage(), e1); //$NON-NLS-1$
					return;
				} catch (NoSuchAlgorithmException e) {
					app.displayErrorDialog(
							Messages.getString("MRUFileAction.ErrorDialog.Title"), e.getMessage(), e); //$NON-NLS-1$
					return;
				}
		...
		if (password != null) {
			try {
				// TODO: change Passworddialog to include a readonly flag; then
				// include readonly here
				app.openFile(fileName, password);
			} catch (Exception e) {
				app.displayErrorDialog(
						Messages.getString("MRUFileAction.OpenError.Title"), Messages.getString("MRUFileAction.OpenError.Message"), e); //$NON-NLS-1$ //$NON-NLS-2$
			}
